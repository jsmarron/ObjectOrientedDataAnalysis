disp('Running MATLAB script file OODAfig7p3.m') ;
%
%    Makes Figure 7.3 of the OODA book,
%    Illustrates L1 Frechet Median
%
%    Copied from OODAbookChpEFigBToyL1FrechetMedian.m
...
%vmedian = rmeanSM(mdata) ;
    %  Note this gives worse FSS
vmedian(1) = median(mdata(:,1)) ;
vmedian(2) = median(mdata(:,2)) ;
...
FSSm = 0 ;
for i = 1:n ;
  FSS1 = FSS1 + abs(mdata(i,1) - CandPoint1(1)) + abs(mdata(i,2) - CandPoint1(2)) ;
  FSSm = FSSm + abs(mdata(i,1) - vmedian(1))^2 + abs(mdata(i,2) - vmedian(2)) ;
end ;
...
subplot(1,2,1) ;    %  make Candidate1 plot
  ...
  hold on ;
    ...
    plot([mdata(:,1)'; mdata(:,1)'], ...
         [mdata(:,2)'; (ones(1,n) * CandPoint1(2))], ...
         'b-','LineWidth',linelw) ;
    plot([mdata(:,1)'; (ones(1,n) * CandPoint1(1))], ...
         [(ones(1,n) * CandPoint1(2)); (ones(1,n) * CandPoint1(2))], ...
         'b-','LineWidth',linelw) ;
...
subplot(1,2,2) ;    %  make median plot
  ...
  hold on ;
    ...
    plot([mdata(:,1)'; (ones(1,n) * vmedian(1))], ...
         [(ones(1,n) * vmedian(2)); (ones(1,n) * vmedian(2))], ...
